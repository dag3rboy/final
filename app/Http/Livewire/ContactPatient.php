...
class ContactPatient extends Component
{
    use WithPagination;
    ...
    {
        // return view('livewire.contact-patient',  [
        //     'patients' => Patient::orderBy('pa_lastname', 'desc')
        //     ->paginate(6),
        // ]);

        return view('livewire.contact-patient', [
            'patients' => Patient::when($this->search, function ($query, $search) {
                return $query->where('pa_firstname', 'LIKE', "%$search%")
                    ->orWhere('pa_lastname', 'LIKE', "%$search%");
            })->paginate(6)
        ]);
    }

    // function to open doctors emails list modal 
    public function openSendEmailModal($pa_email)
    {
        $this->email = $pa_email;
        $this->subject = ""; 
        $this->message = "";
    ...
    public function openSendSMSModal($pa_telephone)
    {
        $this->telephone = $pa_telephone;
        $this->subject = ""; 
        $this->message = "";
    ...
    // function to send sms to patients
